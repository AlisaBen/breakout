...
/**
  ...
  * 道具（暂时不加，快消、炸弹、加速、分裂）
  */
case class ObstacleState(racketId:Int,oId:Int,t:Byte,p:Point,value:Int)//id,类型（racket,brick），位置
...
//    case ObstacleType.airDropBox => new AirDropBox(config,obstacleState)
    ...
    case ObstacleType.fastRemove => new Brick(config,obstacleState)
